...
export interface TradePreview { filename: string; batch: { batch_id: string; file_sha256: string }; summary: Record<string, number>; rows: Array<{ row_ref: string; status: string; candidate: null | Record<string, unknown>; issues: Array<{ code: string }> }> }
export interface MarketPreview { filename: string; batch_id: string; file_sha256: string; summary: Record<string, number>; rows: Array<{ row_number: number; status: string; candidate: null | { date: string; symbol: string; close: number; price_type: string }; issues: Array<{ code: string }> }>; missing_required_dates: [string, string[]][] }
export interface RuntimeInvestments { subject_id: string; account_id: string; as_of: string; data_tier: "authorized_beta"; portfolio_state_status: string; portfolio_state_reason: string | null; summary: { open_episode_count: number; closed_episode_count: number; current_position_count: number }; episodes: Array<{ episode_id: string; instrument_id: string; display_name: string; status: "open" | "closed"; opened_at: string; closed_at: string | null; duration_days: number; duration_kind: "final" | "so_far"; quantity: number | null; average_cost: number | null; valuation_at: string | null; valuation_price: number | null; market_value: number | null }> }
