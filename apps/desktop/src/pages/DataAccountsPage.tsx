...
const summaryLabels: Record<string, string> = {
  accepted_canonical_facts: "Ready to import", new_executions: "New executions",
...
const statusLabels: Record<string, string> = {
  new_execution: "New execution", possible_duplicate: "Possible duplicate", exact_duplicate: "Exact duplicate",
...
export function DataAccountsPage() {
  ...
  const [preview, setPreview] = useState<TradePreview | MarketPreview | null>(null);
  ...
  useEffect(() => {
    ...
    void realUserApi.status(active.subject_id, active.account_id).then(setStatus).catch((value) => setError(String(value)));
  }, [active]);
  ...
  const choose = async () => { const selected = await pickCsv(); if (selected) { setFile(selected); setPreview(null); setError(null); } };
  const runPreview = async () => {
    if (!file) return;
    setBusy(true); setError(null);
    try { setPreview(kind === "trade" ? await realUserApi.previewTrades(params()) : await realUserApi.previewPrices(params())); setDuplicateChoices({}); }
  ...
  const commit = async () => {
    ...
    setBusy(true); setError(null);
    try {
      if (kind === "trade") await realUserApi.commitTrades({ ...params(), expected_file_sha256: (preview as TradePreview).batch.file_sha256, duplicate_choices: duplicateChoices });
      else await realUserApi.commitPrices({ ...params(), expected_file_sha256: (preview as MarketPreview).file_sha256 });
      await data.refresh(); data.setMode("real_user"); setPreview(null); setFile(null);
    } catch (value) { setError(value instanceof Error ? value.message : String(value)); }
    finally { setBusy(false); }
  };
  const remove = async () => {
    if (!active || !window.confirm(t("Delete this local account and all of its imported facts?"))) return;
    await realUserApi.deleteAccount(active.subject_id, active.account_id); await data.refresh();
  };
  ...
  return <div className="page space-y-6 pb-8">
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
      <GlassPanel className="p-5 md:p-6"><SectionHeading eyebrow={t("Import workflow")} title={t(kind === "trade" ? "Import transaction CSV" : "Import historical price CSV")} description={t("Select a local CSV, inspect the deterministic preview, then explicitly confirm.")} />
        <div className="mt-5 flex gap-2"><Button variant={kind === "trade" ? "primary" : "quiet"} onClick={() => { setKind("trade"); setPreview(null); }}>{t("Transactions")}</Button><Button variant={kind === "market" ? "primary" : "quiet"} onClick={() => { setKind("market"); setPreview(null); }}>{t("Historical prices")}</Button></div>
