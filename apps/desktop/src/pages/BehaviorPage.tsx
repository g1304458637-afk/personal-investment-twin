...
import { behaviorMetrics, evidenceRecords } from "@/demo/fixture";
...
const methodExplanations: Record<string, string> = {
  ...
  disposition: "The disposition ratio compares the registered realized-outcome categories. It is experimental because the synthetic sample is limited and does not establish a trait.",
...
function BehaviorDetail({ metric, record, t }: { metric: EvidenceMetric; record: DemoEvidenceRecord; t: (value: string) => string }) {
...
function BehaviorModule({ metric, selected, onSelect, t }: { metric: EvidenceMetric; selected: boolean; onSelect: () => void; t: (value: string) => string }) {
...
export function BehaviorPage() {
  ...
  const selectedRecord = evidenceFor(selectedMetric);
    ...
    <div className="space-y-6 pb-8">
      ...
      <GlassPanel className="p-5 md:p-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><p className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted">{t("Selected synthetic trend")}</p><h2 className="mt-2 text-lg font-semibold text-foreground">{t(selectedMetric.label)}</h2><p className="mt-1 text-sm leading-5 text-muted">{t(methodExplanations[selectedMetric.id])}</p></div>
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_240px] lg:items-end">
          ...
          <div className="rounded-md border border-border/70 bg-white/[0.025] p-4 text-xs leading-5">
            <p className="text-muted">{t("Displayed metric")}</p><p className="mt-1 font-mono text-xl font-semibold text-foreground tabular-nums">{selectedMetric.primary}</p>
            <p className="mt-4 text-muted">{t("Evidence coverage")}</p><p className="text-foreground">N={selectedRecord.observation_count ?? "—"} · {selectedMetric.confidence ? t(selectedMetric.confidence) : t("No CI / range registered")}</p>
