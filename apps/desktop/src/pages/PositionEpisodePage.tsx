...
function pathItemCopy(
  ...
  if (pattern?.patternCode === "high_quantity_during_daily_price_drawdown") {
    ...
    return { title: t("High recorded quantity during the daily price peak-to-trough path"), detail };
  }
...
export function PositionEpisodePage() {
  ...
  const entry = data.mode === "demo" ? (episodeId ? getPositionEpisodeById(episodeId) : null) : runtimeEntry;
  ...
  useEffect(() => {
    ...
    void realUserApi.episode(data.activeAccount.subject_id, data.activeAccount.account_id, episodeId).then((result) => {
      ...
      setRuntimeEntry(adaptRuntimePositionEpisodeEntry(result.entry));
    }).catch((value) => setRuntimeError(value instanceof Error ? value.message : String(value)));
