...
import { adaptSameStock, type CompareView } from "@/data/sameStock";
import { SameStockTimeline } from "@/components/charts/SameStockTimeline";
import { Button } from "@/components/ui/button";
import { useLocale } from "@/locales/LocaleProvider";

export function SameStockComparisonPanel({ view }: { view: CompareView }) {
  ...
  return <section className="space-y-5">
    <div className="grid grid-cols-2 gap-5 border-y border-border py-5">{[view.a, view.b].map((s, i) => <div key={i} className="min-w-0">
      <p className="text-sm text-muted">{i === 0 ? "A" : "B"} · {s.symbol} · {t(s.tier === "synthetic" ? "Example account · Synthetic" : "Shared derived facts")}</p>
    ...
    {view.status !== "unavailable" ? <>
      ...
      <SameStockTimeline view={view} selected={selected} />
...
export function SameStockComparePage() {
  ...
  return <div className="page space-y-5">
    ...
    <SameStockComparisonPanel view={view} />
  </div>;
}
