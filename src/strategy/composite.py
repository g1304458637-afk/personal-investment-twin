...
def validate_user_strategy(raw: Mapping[str, Any]) -> dict:
    ...
    if not isinstance(raw, Mapping) or raw.get("schema_version") != SCHEMA_VERSION:
        raise UserStrategyError("schema_version 必须是 " + SCHEMA_VERSION)
    ...
    if not isinstance(entry_raw, Mapping) or set(entry_raw) - {"all_of"}:
        raise UserStrategyError("entry 只允许 all_of 条件组")
    entry = _conditions_list(entry_raw.get("all_of"), "entry", MAX_ENTRY_CONDITIONS)
    exit_raw = raw.get("exit")
    if not isinstance(exit_raw, Mapping) or set(exit_raw) - {"any_of", "stop_loss_pct"}:
        raise UserStrategyError("exit 只允许 any_of 条件组与 stop_loss_pct")
    ...
    if stop is not None:
        ...
        stop_pct = float(stop)
    if not exit_conditions and stop_pct is None:
        raise UserStrategyError("至少需要一种退出机制（退出条件或止损）")
    ...
    if ranking_raw is not None:
        ...
        ranking = {"factor": ranking_condition["factor"], "params": ranking_condition["params"],
                   "direction": direction}
    windows = [factor_min_history(c["factor"], c["params"]) for c in (*entry, *exit_conditions)]
    ...
    return {
        ...
        "name": name.strip(),
        "entry": {"all_of": entry},
        "exit": {"any_of": exit_conditions, "stop_loss_pct": stop_pct},
        "sizing": sizing,
    ...
    def provider(spec_params: dict[str, float | int | str], data: SimulationData,
        ...
        for instrument in sorted(data.listed_on(day)):
            ...
            if instrument in held:
                ...
                values = [evaluate_condition(c, series, index) for c in normalized["entry"]["all_of"]]
                if all(satisfied for satisfied, _ in values):
                    ranking = normalized["ranking"]
                    strength = values[0][1] if ranking is None else evaluate_factor(
                        ranking["factor"], ranking["params"], series, index)
                    intended = None
                    if normalized["sizing"]["mode"] == "risk_unit" and equity > 0:
                        ...
                        if n and n > 0 and price > 0:
                            sizing = normalized["sizing"]
                            shares = min(sizing["risk_fraction"] * equity / (n * price),
                                         sizing["notional_cap"] * equity / price)
                            intended = float(int(shares / lot) * lot)
                    candidates.append(Signal(
                        instrument=instrument, kind="entry_candidate",
                        strength=strength if strength is not None else 0.0,
                        reason_text="；".join(_condition_statement("入场条件", c)
                                              for c in normalized["entry"]["all_of"]),
                        conditions=(), reason_code="user_entry",
                        intended_quantity=intended))
        direction = (normalized["ranking"] or {}).get("direction", "desc")
