...
def _episode_reports(spec, demo, charts):
    reports = []
    ...
    return reports, user_realized_total, user_episode_count


SLUGS = {
    "toujing_t1_breakout_trend": None,
    "toujing_dual_ma": "dual-ma",
    "toujing_rsi_mean_reversion": "rsi-mean-reversion",
...
def main() -> int:
    ...
    for spec in (build_t1_spec(), build_dual_ma_spec(), build_rsi_mr_spec(), build_turtle_spec()):
        reports, realized_total, episode_count = _episode_reports(spec, demo, charts)
        slug = SLUGS[spec.strategy_id]
        payload = {
        ...
        if slug is None:
            payload["portfolio"] = compare_portfolio(
                {
                    "scope": "SYN_STUDY_SHOWCASE",
                    "realized_episode_count": episode_count,
            ...
            output = OUTPUT.parent / f"strategy-comparison-{slug}.json"
        output.write_text(json.dumps(payload, ensure_ascii=False, indent=1, sort_keys=False,
                                     allow_nan=False) + "\n", encoding="utf-8")
        written[spec.strategy_id] = str(output)
    print(json.dumps({"outputs": written}, ensure_ascii=False))
    return 0
